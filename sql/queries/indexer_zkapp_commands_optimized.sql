WITH
  ...
  zkapp_commands_info AS (
    ...
    FROM
      ...
      LEFT JOIN LATERAL unnest(zca.zkapp_account_updates_ids) WITH ORDINALITY AS au_ref (au_id, au_ord) ON TRUE
      ...
      LEFT JOIN accounts_created AS ac ON ac.block_id=zca.block_id
      AND ac.account_identifier_id=zaub.account_identifier_id
      AND zca.status='applied'
